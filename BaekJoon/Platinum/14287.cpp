...
int tree[400010];
int lazy[400010];
vector<int> subList[100001]; // 각 노드의 부하 직원들을 저장

int dfs_start_index[100001];
int dfs_end_index[100001];
int arr[100001];
int dfs_arr[100001];
int dfs_count;
...
int main()
{
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        if (order == 1)
        {
            long long x;
            cin >> x;
            update_range(1, 1, N, dfs_start_index[a], dfs_start_index[a], x);
        } else
        {
            cout << query(1, 1, N, dfs_start_index[a], dfs_end_index[a]) << "\n";
        }
    }
